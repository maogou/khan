...
func (k *Khan) LoginQrCode(ctx context.Context, req transform.GetLoginQrCodeRequest) (*transform.GetLoginQrCodeResponse, error) {
	log := help.GetQidLog(ctx)
	log.Info().Msg("调用LoginQrCode方法")

	resp, err := k.client.R().SetBody(req).SetResult(&transform.GetLoginQrCodeResponse{}).Post(loginQrCode)
...
func (k *Khan) CheckLoginQrCode(ctx context.Context, req transform.CheckLoginRequest) (*transform.CheckLoginResponse, error) {
	log := help.GetQidLog(ctx)
	log.Info().Msg("调用CheckLoginQrCode方法")

	resp, err := k.client.R().SetBody(req).SetResult(&transform.CheckLoginResponse{}).Post(checkLoginQrCode)
